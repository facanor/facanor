...
const Toolbar = (anno, container) => {
    ...
    const clearActive = () => {
        ...
        if (currentActive) removeClass(currentActive, "active");
    };
    ...
    const createButton = (toolId, isActive) => {
        ...
        if (icon) {
            ...
            if (isActive) button.className = `a9s-toolbar-btn ${toolId} active`;
            else button.className = `a9s-toolbar-btn ${toolId}`;
            ...
            button.addEventListener("click", () => {
                ...
                if (isOSDPlugin) anno.setDrawingEnabled(true);
            });
    ...
    if (isOSDPlugin) anno.on("createSelection", clearActive);
